...
const cooldownTime = 1000 * 60 * 60 * 12;
...
function findRewards(winner, loser) {
	...
	// Check if losing team was enemy
	else if (loser.type === "enemy") {
		rewards = rewardList.official[loser.arenaRank];
		upRank = parseInt(loser.arenaRank);
	}
	// Neither team was, spar
	else {
...
// Create user
function createData(teamID, teamName) {
	const data = {
		id: teamID,
		team: teamName,
		level: 0,
	...
	// Save data
	arenasql.prepare("INSERT OR REPLACE INTO arena (id, tag, team, level, SP, coins, nextBattle, battleTime)"
		+ " VALUES (@user, @tag, @team, @level, @SP, @coins, @nextBattle, @battleTime);").run(data);
}

// Sets cooldown time
function setTime(userID) {
	arenasql.prepare(`UPDATE arena SET battleTime = ${(Date.now() + cooldownTime)} WHERE user = ?`).run(userID);
}

// Gets cooldown time
function getTime(userID) {
	const data = arenasql.prepare(`SELECT user, battleTime FROM arena WHERE user = ?`).get(userID);
	return data.battleTime;
}

// Views user data
function viewData(msg, client) {
	const data = arenasql.prepare(`SELECT * FROM arena WHERE user = ${msg.author.id}`).get();
	if (!data) {
	...
	// Embed
	const arenadataEmbed = new Discord.RichEmbed()
...
function addRewards(userID, rewards, upRank) {
	...
	console.log(rewards);
	// Get or create data
	const data = arenasql.prepare(`SELECT user, SP, coins, nextBattle FROM arena WHERE user = ${userID}`).get();
	if (!data) {
		return;
	}
...
function deleteUser(userID) {
	try {
		const data = arenasql.prepare(`DELETE FROM arena WHERE user = ${userID};`).run();
		return data;
	}
	catch (err) {
		console.log(err);
...
function getNextMatch(userID) {
	const data = arenasql.prepare(`SELECT user, nextBattle FROM arena WHERE user = ${userID}`).get();
	return data.nextBattle;
}

// Check for levelup
function checkLevelUp(userID) {
	const data = arenasql.prepare(`SELECT user, SP, level FROM arena WHERE user = ?`).get(userID);
